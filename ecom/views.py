...
def search_view(request):
    ...
    products = models.Product.objects.filter(
        ...
        Q(category__name__icontains=query) |  # Navigate the ForeignKey relationship to the category name
...
def add_to_cart_view(request, pk):
    ...
    # For cart counter, fetching products ids added by the customer from cookies
    if 'product_ids' in request.COOKIES:
        product_ids = request.COOKIES['product_ids']
        counter = product_ids.split('|')
        product_count_in_cart = len(set(counter))
    ...
    if str(pk) not in product_ids:
        # Add the new product ID to the existing ones
        if product_ids == "":
            product_ids = str(pk)
        else:
            product_ids = product_ids + "|" + str(pk)

        # Set the updated product_ids back to the cookie
        response = render(request, 'ecom/products.html', {'products': products, 'product_count_in_cart': product_count_in_cart + 1})
        response.set_cookie('product_ids', product_ids)
        
        product = models.Product.objects.get(id=pk)
        messages.info(request, product.name + ' added to cart successfully!')
    else:
        # If the product is already in the cart, just render the page without changing the cookie
...
def remove_from_cart_view(request, pk):
    ...
    if 'product_ids' in request.COOKIES:
        ...
        if value == "":
            # Clear the 'product_ids' cookie if the cart is empty
            response.delete_cookie('product_ids')
        else:
            response.set_cookie('product_ids', value)
        return response

    # If 'product_ids' key is not present in the request.COOKIES, return an empty cart
    # and clear the 'product_ids' cookie
    response = render(request, 'ecom/cart.html', {'products': [], 'total': total, 'product_count_in_cart': 0})
    response.delete_cookie('product_ids')
    return response
...
def success_view(request):
    ...
    return render(request,'ecom/success.html',{'product_count_in_cart':product_count_in_cart})
"""
def products_view(request):
    products=models.Product.objects.all()
    if 'product_ids' in request.COOKIES:
